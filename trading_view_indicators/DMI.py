import pandas as pd
import numpy as np
from tradingview_indicators.moving_average import rma
...
class DMI:
    ...
    def my_true_range(self) -> pd.Series:
        ...
        return tr.fillna(0)  # Avoid returning all-NaN Series

    def atr(self, length: int = 14) -> pd.Series:
        tr = self.my_true_range()
        if tr.dropna().empty or len(tr.dropna()) < length:
            return pd.Series(index=tr.index, data=np.nan, name="ATR")
